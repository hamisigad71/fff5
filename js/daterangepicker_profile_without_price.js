//https://longbill.github.io/jquery-date-range-picker/ for example
! function(t, e) {
    if ("function" == typeof define && define.amd) define(["moment", "jquery", "exports"], function(a, i, s) {
        t.daterangepicker = e(t, s, a, i)
    });
    else if ("undefined" != typeof exports) {
        var a = require("moment"),
            i = "undefined" != typeof window ? window.jQuery : void 0;
        if (!i) try {
    ...
    var s = function(t, e, s) { //his.singleDatePicker =!1 to show 2 month calender
        ...
        if ("undefined" == typeof e.startDate && "undefined" == typeof e.endDate && i(this.element).is("input[type=text]")) {
            var o = i(this.element).val(),
                c = o.split(this.locale.separator);
    ...
    s.prototype = {
        ...
        updateMonthsInView: function() {
            if (this.endDate) {
                ...
                this.leftCalendar.month = this.endDate.clone().date(2), this.linkedCalendars || this.endDate.month() == this.startDate.month() && this.endDate.year() == this.startDate.year() ? this.rightCalendar.month = this.startDate.clone().date(2).add(1, "month") : this.rightCalendar.month = this.endDate.clone().date(2)
        ...
        renderCalendar: function(t) {
            ...
            var e = "left" == t ? this.leftCalendar : this.rightCalendar,
                ...
                c = a([n, s, 1]),
                d = a([n, s, o]),
                m = a(c).subtract(1, "month").month(),
                f = a(c).subtract(1, "month").year(),
                p = a([f, m]).daysInMonth(),
                u = c.day(),
                e = [];
            ...
            Y += "<thead>", Y += "<tr>", this.showWeekNumbers && (Y += "<th><span style='display:none;'>Prev</span></th>"), Y += b && !b.isBefore(e.firstDay) || this.linkedCalendars && "left" != t ? "<th><span style='display:none;'>Prev</span></th>" : '<th class="prev available"><em class="fa fa-chevron-left glyphicon glyphicon-chevron-left"></em></th>';
            if(this.locale.yearSuffix!=''){
				var P = e[1][1].format(" YYYY")+this.locale.yearSuffix + this.locale.monthNames[e[1][1].month()];
			}else{
            ...
            if (Y += '<th colspan="5" class="month">' + P + "</th>", Y += C && !C.isAfter(e.lastDay) || this.linkedCalendars && "right" != t && !this.singleDatePicker ? "<th><span style='display:none;'>Next</span></th>" : '<th class="next available"><em class="fa fa-chevron-right glyphicon glyphicon-chevron-right"></em><span style="display:none;">Next</span></th>', Y += "</tr>", Y += "<tr>", this.showWeekNumbers && (Y += '<th class="week">' + this.locale.weekLabel + "</th>"), i.each(this.locale.daysOfWeek, function(t, e) {
                    Y += "<th>" + e + "</th>"
            ...
            for (var y = 0; 6 > y; y++) {
                ...
                for (var k = 0; 7 > k; k++) {
                    var j = [];
                    e[y][k].isSame(new Date, "day") && j.push("today"), e[y][k].isoWeekday() > 5 && j.push("weekend"), e[y][k].month() != e[1][1].month() && j.push("off"), this.minDate && e[y][k].isBefore(this.minDate, "day") && j.push("off", "disabled"), C && e[y][k].isAfter(C, "day") && j.push("off", "disabled"), this.isInvalidDate(e[y][k]) && j.push("off", "disabled"), e[y][k].format("YYYY-MM-DD") == this.startDate.format("YYYY-MM-DD") && j.push("active", "start-date"), null != this.endDate && e[y][k].format("YYYY-MM-DD") == this.endDate.format("YYYY-MM-DD") && j.push("active", "end-date"), null != this.endDate && e[y][k] > this.startDate && e[y][k] < this.endDate && j.push("in-range");
                    for (var H = "", O = !1, D = 0; D < j.length; D++) H += j[D] + " ", "disabled" == j[D] && (O = !0);
					//Rajiv
					var d = new Date(e[y][k]);
					var m = d.getMonth() + 1;
					//Rajiv
                    //dharmveer
					if(e[y][k].month() != e[1][1].month()){
						/*
						 O || (H += "available"), Y += '<td  class="' + H.replace(/^\s+|\s+$/g, "") + '" data-title="r' + y + "c" + k + '">'  + e[y][k].date() +' '+( H.match(/start-date/g)?'<span style="display:none" class="cal_caption">'+CHECKIN.toUpperCase():'')+'</span>'+' '+( H.match(/end-date/g)?'<span style="display:none" class="cal_caption">'+CHECKOUT.toUpperCase()+'</span>':'')+'x</td>'
						 */
						 O || (H += "available"), Y += '<td  class="off disabled" data-title="r' + y + "c" + k + '" style="text-decoration:none !important;">&nbsp;</td>' 
					}else{
						O || (H += "available"), Y += '<td  class="' + H.replace(/^\s+|\s+$/g, "") + '" data-title="r' + y + "c" + k + '">'  + e[y][k].date() +' '+( H.match(/start-date/g)?'<span style="display:none" class="cal_caption">'+CHECKIN.toUpperCase():'')+'</span>'+' '+( H.match(/end-date/g)?'<span style="display:none" class="cal_caption">'+CHECKOUT.toUpperCase()+'</span>':'')+'</td>'
					}
        ...
        move: function() {
            ...
            this.parentEl.is("body") || (e = {
            ...
            }, a = this.parentEl[0].clientWidth + this.parentEl.offset().left), t = "up" == this.drops ? this.element.offset().top - this.container.outerHeight() - e.top : this.element.offset().top + this.element.outerHeight() - e.top, this.container["up" == this.drops ? "addClass" : "removeClass"]("dropup"), "left" == this.opens ? (this.container.css({}), this.container.offset().left < 0 && this.container.css({})) : "center" == this.opens ? (this.container.css({
                top: t,
                ...
                right: "auto",
                left: 9
            })) : (this.container.css({}), this.container.offset().left + this.container.outerWidth() > i(window).width() && this.container.css({}))
        },
        show: function(t) {
            this.isShowing || (this._outsideClickProxy = i.proxy(function(t) {
                this.outsideClick(t)
            }, this), i(document).on("mousedown.daterangepicker", this._outsideClickProxy).on("touchend.daterangepicker", this._outsideClickProxy).on("click.daterangepicker", this._outsideClickProxy).on("focusin.daterangepicker", this._outsideClickProxy), i(window).on("resize.daterangepicker", i.proxy(function(t) {
                this.move(t)
            ...
            if (!this.container.find("input[name=daterangepicker_start]").is(":focus") && !this.container.find("input[name=daterangepicker_end]").is(":focus") && i(t.target).hasClass("available")) {
                ...
                this.endDate || this.container.find(".calendar td").each(function(t, e) {
                    if (!i(e).hasClass("week")) {
                        var a = i(e).attr("data-title"),
                            s = a.substr(1, 1),
                            n = a.substr(3, 1),
                            c = i(e).parents(".calendar"),
                            d = c.hasClass("left") ? h.calendar[s][n] : l.calendar[s][n];
